...
export const NavBar = () => {
    ...
    const navLinkStyle = "p-4 lg:p-1 size-4.5  w-full  lg:w-auto";
    ...
    const NavItemFisrtRow = () => {
        return  <div className={`${ isOpen ? 'flex mt-2  w-full bg-(--main-color) gap-4' : 'hidden'} lg:flex flex-col  items-center justify-center  
         text-center  transition-all duration-300 ease-in-out z-10  lg:flex-row lg:justify-between lg:h[52px] lg:gap-6 `} >
           <Link href={RouterPath.Home} className={`hidden lg:block ${navLinkStyle}`}>Welcome</Link>
            <Link  href={RouterPath.OurMenu} className={navLinkStyle}>Our Menu</Link>
            <Link  href={RouterPath.Franchise} className={navLinkStyle}>Franchise</Link>
            <Link  href={RouterPath.Contact} className={navLinkStyle}>Contact</Link>
         </div>
    };
    ...
    const NavItemSecondRow = () => {
        return (
            <div className={`${ isOpen ? 'flex mt-2 ' : 'hidden' } lg:flex flex-col  lg:flex-row justify-between items-center  h-auto gap-4 lg:gap-8 transition-all duration-300 ease-in-out z-10 pt-4 lg:pt-0 `}>
